...
def get_tldr(split: str, human_prefix: str, human_suffix: str, assistant_prefix: str, assistant_suffix: str) -> Dataset:
    """
    Load the OpenAI Summarization Dataset from Huggingface and convert it into to a dictionary of Examples.
    For this dataset, the SFT text is the preferred response.
    ...
    Returns:   
        A Dataset instance.
    """
    rank0_print(f'Loading OpenAI summarization dataset ({split} split) from Huggingface...')
    dataset = datasets.load_dataset('openai/summarize_from_feedback', "comparisons", split=('validation' if split == 'test' else 'train'))
    if on_rank0():
        dataset = tqdm.tqdm(dataset, desc='Processing OpenAI summarization dataset')

    data = Dataset('tldr')
    for row in dataset:
        # Constructing the prompt
        post_content = row['info']['post'] if row['info']['post'] is not None else ''
        prompt = human_prefix + post_content + human_suffix + assistant_prefix

        # Choosing the preferred response
        preferred_summary = row['summaries'][row['choice']]['text'] + assistant_suffix
        alternative_summary = row['summaries'][1 - row['choice']]['text'] + assistant_suffix

        responses = [preferred_summary, alternative_summary]
