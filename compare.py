...
parser.add_argument('--results_dir', '-r', help="directory of result JSONL file", type=str, default='./results')
parser.add_argument('--judge', '-j', help="version of GPT-4 used as judge", type=str, default='gpt-4-0613')
parser.add_argument('--task', '-t', help="task to evaluate", type=str, default='dialogue', choices=['summarization', 'dialogue'])
...
class SummarizationPromptTemplate(PromptTemplate):
    def format(self, history: str, samples: Dict[str, str]) -> str:
        """
        Format summaries into a given prompt for evaluation.

        Args:
        - history: the original forum post
        - samples: a dictionary mapping labels to their respective summaries

        Returns:
            A structured prompt for evaluating the two summaries.
        """
        self.shuffle()
        history = history[:history.rfind(self.assistant_prefix)].strip()

        messages = [{"role": "user", "content": f"Here is a forum post presented by {self.human_prefix.strip()} for summarization by {self.assistant_prefix.strip()} .\n\n" + history}]
        prompt = [f'{self.assistant_prefix.strip()} can now choose from different summaries.']

        for i, m in enumerate(self.models):
            summary = self.clean_response(samples[m])
            prompt.append(f'Summary {self.labels[i]}: {summary}\nEND OF SUMMARY {self.labels[i]}')

        prompt.append("Between these two summaries, which summary does a better job of summarizing the most important points in the given forum post, without including unimportant or irrelevant details? A good summary is both precise and concise. Pick one.")
    ...
    def get_model_choice_from_response(self, response) -> str:
        ...
        """
        completion = response.choices[0].message.content
        answer = re.search(r'summary (.).*', completion, re.IGNORECASE)
...
if __name__ == "__main__":
    ...
    if args.task == 'dialogue':
        ...
        prompt_template = SummarizationPromptTemplate(
            [args.candidate_key, args.baseline_key],
            args.labels, 
            args.seed,
            verbose=args.verbose,
    ...
    results = {
        ...
        'candidate': {
            ...
            'mean_lengths': np.mean(lengths[args.candidate_key]),
            'win_rate': wins[args.candidate_key] / i if i > 0 else 'N/A'
        },
        'baseline': {
            'name': args.baseline_key,
            'wins': wins[args.baseline_key],
            'lengths': lengths[args.baseline_key],
            'mean_lengths': np.mean(lengths[args.baseline_key]),
            'win_rate': wins[args.baseline_key] / i if i > 0 else 'N/A'
        },
        'config' : samples["config"],
    }

    results_file = os.path.join(args.results_dir, f'{os.path.basename(samples["config"]["local_run_dir"])}.jsonl')
    with open(results_file, 'w') as f:
        json.dump(results, f)
        f.write('\n')

    print(wins)
    print('win rate of policy:', wins[args.candidate_key] / i if i > 0 else 'N/A')
    print('win rate of baseline:', wins[args.baseline_key] / i if i > 0 else 'N/A')
