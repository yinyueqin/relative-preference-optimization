...
class BasicTrainer(object):
    def __init__(self, 
        ...
        if self.config.loss.name == "rpo-paired" or self.config.loss.name == "rpo-unpaired":
            self.sentence_transformer = SentenceTransformer(self.config.loss.sentence_transformer_name_or_path)
            rank0_print(f'Loaded sentence transformer')
            self.sentence_transformer.eval()
            self.sentence_transformer.to(self.rank)
    ...
    def sample(self, include_original_prompt=False) -> List[Dict[str, str]]:
        ...
        for eval_batch in self.eval_batches:
            ...
            # if self.n_samples is not None and len(all_prompts) > self.n_samples:
            #     break
            # else:
    ...
    def train(self):
        ...
        for batch in self.train_iterator:
            ...
            if self.example_counter % self.config.eval_every == 0 and (self.example_counter > 0 or self.config.do_first_eval):
                ...
                for eval_batch in (tqdm.tqdm(self.eval_batches, desc='Computing eval metrics') if self.rank == 0 else self.eval_batches):
                    # Skip the loop if all statuses are 'chosen' or 'rejected'
                    if 'status' in eval_batch:
                        if len(set(eval_batch['status'])) == 1:
                            continue
                    local_eval_batch = slice_and_move_batch_for_device(eval_batch, self.rank, self.world_size, self.rank)
                    with torch.no_grad():
                        _, eval_metrics = self.get_batch_metrics(local_eval_batch, mode='eval')
...
class UnpairedPreferenceTrainer(BasicTrainer):
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], mode: str=None):
        ...
        if self.reference_model is None:
            ...
            if 'rpo-unpaired' in self.config.loss.name:
                prompts = batch['prompt_text']
                prompts_emb = self.sentence_transformer.encode(prompts)
                status = batch['status']
                losses, chosen_rewards, rejected_rewards = self.loss(policy_chosen_logps, policy_rejected_logps, reference_chosen_logps, reference_rejected_logps, prompts_emb=prompts_emb, status=batch['status'])
            else:
        ...
        metrics[f'rewards_{mode}/accuracies'] = (chosen_rewards > rejected_rewards).float().cpu().numpy().tolist()
...
class PairedPreferenceTrainer(BasicTrainer):
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], mode: str=None):
        ...
        if self.reference_model is None:
            ...
            with torch.no_grad():
                reference_chosen_logps, reference_rejected_logps = self.forward(self.reference_model, batch)
            if self.config.loss.name == 'rpo-paired':
                prompts = batch['prompt_text']
                prompts_emb = self.sentence_transformer.encode(prompts)
                losses, chosen_rewards, rejected_rewards = self.loss(policy_chosen_logps, policy_rejected_logps, reference_chosen_logps, reference_rejected_logps, prompts_emb=prompts_emb)
            else:
                losses, chosen_rewards, rejected_rewards = self.loss(policy_chosen_logps, policy_rejected_logps, reference_chosen_logps, reference_rejected_logps)
...
class CDPOTrainer(PairedPreferenceTrainer):
       def loss(self,
             ...
             reference_chosen_logps: torch.FloatTensor,
             reference_rejected_logps: torch.FloatTensor) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        """Compute the Conservative DPO loss for a batch of policy and reference model log probabilities."""
...
class RPODiagonalTrainer(PairedPreferenceTrainer):
       ...
       def loss(self,
             ...
             reference_chosen_logps: torch.FloatTensor,
             reference_rejected_logps: torch.FloatTensor) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        """Compute the Paired RPO-Diagonal loss for a batch of policy and reference model log probabilities."""

        pi_logdiffs = policy_chosen_logps.view(-1, 1) - policy_rejected_logps.view(1, -1)
        ref_logdiffs = reference_chosen_logps.view(-1, 1) - reference_rejected_logps.view(1, -1)

        logits = pi_logdiffs - ref_logdiffs
        
        all_losses = -F.logsigmoid(self.config.loss.beta * logits)
        
        diagonal_losses = all_losses.diag()
        
        off_diagonal_mask = ~torch.eye(*logits.shape, dtype=torch.bool)
        off_diagonal_losses = all_losses[off_diagonal_mask].view(logits.size(0), -1).mean(dim=1)
        
        combined_losses = (self.config.loss.diagonal_weight * diagonal_losses) + ((1 - self.config.loss.diagonal_weight) * off_diagonal_losses)

        chosen_rewards = self.config.loss.beta * (policy_chosen_logps - reference_chosen_logps).detach()
        rejected_rewards = self.config.loss.beta * (policy_rejected_logps - reference_rejected_logps).detach()

        return combined_losses, chosen_rewards, rejected_rewards
...
class RPOTrainer(PairedPreferenceTrainer):
       def loss(self,
             ...
             prompts_emb: Optional[torch.FloatTensor] = None) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        """Compute the Paired RPO loss for a batch of policy and reference model log probabilities."""
        ...
        logits = chosen_logdiffs.view(-1, 1) - rejected_logdiffs.view(1, -1)
        ...
        if prompts_emb is not None:
            ...
            distance_weights = F.softmax(-cos_distance / self.config.loss.distance_temperature, dim=1)
        else:
...
class RPOUnpairedTrainer(UnpairedPreferenceTrainer):
       def loss(self,
             ...
             reference_chosen_logps: torch.FloatTensor,
             reference_rejected_logps: torch.FloatTensor,
             prompts_emb: Optional[torch.FloatTensor] = None,
             status: Optional[List[str]] = None) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        """Compute the Unpaired RPO loss for a batch of policy and reference model log probabilities."""
        status_tensor = torch.tensor([1 if s == 'chosen' else 0 for s in status])
        chosen_indices = status_tensor == 1
        rejected_indices = status_tensor == 0

        if policy_chosen_logps.shape[0] == 0 or policy_rejected_logps.shape[0] == 0:
            return torch.Tensor([0]).to(self.policy_dtype).to(self.device), torch.Tensor([0]).to(self.policy_dtype).to(self.device), torch.Tensor([0]).to(self.policy_dtype).to(self.device)
        ...
        if prompts_emb is not None:
            prompts_emb = torch.tensor(prompts_emb, dtype=policy_chosen_logps.dtype)
            chosen_emb = prompts_emb[chosen_indices]
            rejected_emb = prompts_emb[rejected_indices]

            norm_chosen_emb = F.normalize(chosen_emb, p=2, dim=1)
            norm_rejected_emb = F.normalize(rejected_emb, p=2, dim=1)

            cos_distance = 1 - torch.mm(norm_chosen_emb, norm_rejected_emb.t())
            distance_weights = F.softmax(-cos_distance / self.config.loss.distance_temperature, dim=1)
        else:
            distance_weights = torch.ones_like(logits)
...
class KTOTrainer(UnpairedPreferenceTrainer):
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], mode: str=None):
        ...
        metrics[f'rewards_{mode}/KL_estimate'] = [all_KL.item()] if all_KL.nelement() == 1 else all_KL.float().cpu().numpy().tolist()
        metrics[f'loss/{mode}'] = all_devices_losses.float().cpu().numpy().tolist()
